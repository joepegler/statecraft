...
Because TypeScript understands the fixture order, it can infer which fields are available in your `async ({ ... })` callback. In the example above, `chains.default` includes viem clients after `withChain()`, and `chains.default.wallet` after `withFundedWallet()`.
