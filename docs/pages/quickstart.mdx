...
test(
  ...
  scenario(
    ...
    async ({ chains }) => {
      ...
      const balance = await ch.publicClient.getBalance({
        address: ch.wallet!,
      });
...
This test starts a fresh local Anvil chain, funds a scenario wallet with `withFundedWallet` on `ctx.chains.default`, reads the wallet balance with `chains.default.publicClient.getBalance`, and asserts it.
