...
describe.sequential("external runtime lifecycle owned by the test file", () => {
  ...
  test(
    ...
    scenario(
      ...
      async ({ chains }) => {
        ...
        const start = await ch.publicClient.getBlockNumber();
        await ch.testClient.mine({ blocks: 2 });
        sharedBlock = await ch.publicClient.getBlockNumber();
  ...
  test(
    ...
    scenario(
      withExternalRuntime({ runtime: handle }),
      async ({ chains }) => {
        expect(await chains!.default.publicClient.getBlockNumber()).toBe(sharedBlock);
      },
    ),
  );
  ...
  test(
    ...
    scenario(
      ...
      async ({ chains }) => {
        const ch = chains!.default;
        const before = await ch.publicClient.getBlockNumber();
        await ch.testClient.mine({ blocks: 5 });
        expect(await ch.publicClient.getBlockNumber()).toBe(before + 5n);
      },
    ),
  );
