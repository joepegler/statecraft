...
- [`withExternalRuntime({ runtime, runtimeMode?, clients? })`](/fixtures/runtime/withExternalRuntime)

## Additional wiring
- [`withBundler({ entryPoint, mode? })`](/fixtures/runtime/withBundler)
- `withBundler` requires fork runtime mode (`withFork(...)`, or external runtime declared with `runtimeMode: "fork"`).
