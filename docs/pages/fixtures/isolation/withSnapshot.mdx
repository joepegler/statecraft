...
test(
  ...
  scenario(
    ...
    async ({ chains }) => {
      ...
      expect(await ch.publicClient.getBlockNumber()).toBeGreaterThan(0n);
    },
  ),
);
```
...
describe("shared runtime with snapshot isolation", () => {
  ...
  test(
    ...
    scenario(
      ...
      async ({ chains }) => {
        const ch = chains!.default;
        const before = await ch.publicClient.getBalance({ address: recipient });
        ...
        const after = await ch.publicClient.getBalance({ address: recipient });
  ...
  test(
    ...
    scenario(
      ...
      async ({ chains }) => {
        const ch = chains!.default;
        const balance = await ch.publicClient.getBalance({ address: recipient });
