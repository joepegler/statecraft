---
layout: landing
title: Deterministic Ethereum integration testing for TypeScript and viem
description: Statecraft is a composable scenario runtime for TypeScript EVM tests. Build explicit setup with withChain, withFork, withFundedWallet, and related fixtures so test bodies stay focused on behavior. Examples use Vitest; other test runners work too.
---

import { HomePage, Button } from "vocs/components";

<HomePage.Root>
  <img
    src="/logo.png"
    alt="Statecraft"
    style={{ width: 78, height: 78, borderRadius: 18 }}
  />
  <HomePage.Tagline>
    Ethereum integration tests without setup glue code.
  </HomePage.Tagline>
  <HomePage.Description>
    Statecraft turns brittle hooks and helper stacks into explicit, typed scenario fixtures with isolated chains per file, so setup stays deterministic and suites run in parallel.
  </HomePage.Description>
  <HomePage.Buttons>
    <HomePage.Button href="/quickstart" variant="accent">
      Get Started
    </HomePage.Button>
    <HomePage.Button href="/overview">Why it works</HomePage.Button>
    <Button href="https://github.com/joepegler/statecraft">
      GitHub
    </Button>
  </HomePage.Buttons>
</HomePage.Root>

## Why teams switch

- Hidden setup order becomes explicit fixture order in `scenario(...)`.
- Ad hoc fork setup becomes pinned, deterministic chain state.
- Independent files can run in parallel on separate runtimes for faster suites.
- Helper sprawl becomes one readable test pipeline.

<div style={{ position: "relative", overflow: "hidden", marginTop: 28 }}>

  <div
    style={{
      position: "relative",
      zIndex: 1,
      maxWidth: 1000,
      margin: "0 auto",
      padding: "0 16px",
    }}
  >
    ```ts
    import { expect, test } from "vitest";
    import { parseEther, parseUnits } from "viem";
    import {
      scenario,
      withFork,
      withFundedWallet,
      withErc20Balance,
    } from "@st8craft/core";

    test(
      "pinned fork + funded wallet + USDC balance",
      scenario(
        withFork({
          rpcUrl: process.env.VITE_RPC_URL!,
          blockNumber: 22_000_000n,
        }),
        withFundedWallet({ balance: parseEther("1") }),
        withErc20Balance({
          token: "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48", // USDC
          amount: parseUnits("1000000", 6),
        }),
        async ({ chains, publicClient }) => {
          // default chain wallet now has both 1 ETH and seeded USDC
          const balance = await publicClient.getBalance({ address: chains!.default.walletClient.account!.address });
          expect(balance).toBe(parseEther("1"));
        },
      ),
    );
    ```
  </div>
</div>
